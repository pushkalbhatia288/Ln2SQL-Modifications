...
class SelectParser(Thread):
    def __init__(self, columns_of_select, tables_of_from, phrase, count_keywords, sum_keywords, average_keywords,
                 max_keywords, min_keywords, distinct_keywords, database_dico, database_object, group_by_phrase):
        ...
        for table_of_from in self.tables_of_from:  # for each query
            ...
            if number_of_select_column == 0:
                ...
                for count_keyword in self.count_keywords:
                    ...
                    #count_keyword != '' added by myself
                    if count_keyword in lower_self_phrase and count_keyword != '':
                        select_type.append('COUNT')
                        #pass
                #added by myself to include the group by with select
                if self.gbp != []:
                    for x in self.gbp:
                        self.select_object.add_column(x, "")
                ...
                column_of_select_index = -1 #added by myself to choose the right column for aggregates
                for i in range(0, len(select_phrases)):  # for each select phrase (i.e. column processing)
                    ...
                    if (i != len(select_phrases)-1):
                        #if statement added by myself in order to handle index outof error for multiple aggregates.------------
                        flag = 0
                        for word in self.columns_of_select:
                            if word in phrase:
                               flag = 1
                               break
                            else:
                               for x in (self.database_object.get_column_with_this_name(word).equivalences):
                                   if x in phrase:
                                      flag = 1
                                      break 

                        if phrase and flag == 1:
                            column_of_select_index += 1
                            #-----------------------------------------------
                            column = self.get_column_name_with_alias_table(self.columns_of_select[column_of_select_index], table_of_from)
                    ...
                    for table_name in self.database_dico:
                        ...
                        for column in columns:
                            if (phrase[i] == column.name) or (phrase[i] in column.equivalences):
                                ...
                                #Added by myself
                                self.gbp.append(column_with_alias)
...
class Parser:
    ...
    def parse_sentence(self, sentence, stopwordsFilter=None):
        ...
        #print(start_phrase, med_phrase, end_phrase)
            ...
            for idx, x in enumerate(irext_list):
                ...
                if x == maverickjoy_like_assigner:
                    if index < len(irext_list) and irext_list[index] != maverickjoy_like_assigner and irext_list[index] !=\
                        ...
                        #added by myself ---------
                        if  " ends with " in original_sentence:
                            columns_of_values_of_where.append(str("'%" + str(irext_list[index]).replace('<_>', ' ') + "'"))
                        elif " starts with " in original_sentence or " starts from " in original_sentence:
                            columns_of_values_of_where.append(str("'" + str(irext_list[index]).replace('<_>', ' ') + "%'"))
                            #----------------
                        else:
...
#----------------Added by myself------------------------
        ...
        bigram_list = [" ".join(x) for x in bigrams]
        
        columns = self.database_object.get_table_by_name(table_name).get_columns()
        column_names = []
        for column in columns:
            column_names.append(column.name)
            if column.equivalences:
                for equivalence in column.equivalences:
                    column_names.append(equivalence)
        for word in bigram_list:
            if word in self.database_dico or word in self.database_object.get_table_by_name(table_name).equivalences or word in column_names:
                for a in range(len(words)):
                    if words[a] == word.split()[0]:
                       words[a] = word
                       if a < len(words):
                          del words[a+1]
                       break
#------------------------------------------------------
        ...
        try:
            ...
            select_parser = SelectParser(columns_of_select, tables_of_from, select_phrase, self.count_keywords,
                                         ...
                                         self.distinct_keywords, self.database_dico, self.database_object, group_by_parser.get_gbp())
